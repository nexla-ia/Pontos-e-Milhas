...
function ActivityModal({ isOpen, activity, onClose, onSave, loading }: ActivityModalProps) {
      ...
      <div className="bg-white rounded-lg p-6 w-full max-w-md">
        <div className="flex justify-between items-center mb-4">
          <h3 className="text-lg font-semibold text-gray-900">
            {activity ? 'Editar Atividade' : 'Cadastrar Nova Atividade'}
          </h3>
          <button
            onClick={onClose}
...
export default function ListarAtividadesPage() {
  ...
  const [confirmModal, setConfirmModal] = useState<{
    ...
    activityId: string;
    activityName: string;
  }>({ isOpen: false, activityId: '', activityName: '' });
  ...
  const handleDeleteActivity = (activityId: string, activityName: string) => {
    setConfirmModal({ isOpen: true, activityId, activityName });
  };
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      setConfirmModal({ isOpen: false, activityId: '', activityName: '' });
    }
  };
  ...
  return (
    <div className="p-6">
      ...
      <div className="mt-4 text-sm text-gray-600">
        Total de atividades: {totalItems}
      </div>
      ...
      <ConfirmModal
        ...
        message={`Tem certeza que deseja excluir a atividade "${confirmModal.activityName}"? Esta ação não pode ser desfeita.`}
        confirmText="Excluir"
        cancelText="Cancelar"
        onConfirm={confirmDelete}
        onCancel={() => setConfirmModal({ isOpen: false, activityId: '', activityName: '' })}
      />
    </div>
  );
}