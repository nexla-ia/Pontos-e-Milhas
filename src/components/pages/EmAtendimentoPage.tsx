...
export default function EmAtendimentoPage() {
  ...
  const loadOrders = async () => {
    try {
      ...
      // Buscar pedidos com status "in_service" e fazer join com users e agencies
      ...
      const transformedOrders: Order[] = (ordersData || []).map(order => {
        const serviceStartedAt = order.updated_at; // Assumindo que updated_at é quando passou para in_service
        const serviceDate = new Date(serviceStartedAt);
        const now = new Date();
    ...
    <div className="p-6">
      ...
      <div className="flex justify-between items-center mb-6">
        <nav className="text-sm text-gray-600">
          ...
          <span className="mx-2">›</span>
          <span className="text-gray-900 font-medium">Em Atendimento</span>
        </nav>
        ...
        <div className="flex flex-wrap gap-4">
          {Object.entries(columnVisibility).map(([key, visible]) => {
            const labels = {
              ...
              total: 'Total',
              origem: 'Origem',
              tempoAtendimento: 'Tempo em Atendimento'
            };
        ...
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              ...
              {columnVisibility.tempoAtendimento && (
            ...
            {currentItems.length === 0 ? (
              <tr>
                <td colSpan={11} className="px-6 py-8 text-center text-gray-500">
                  Nenhuma OP em atendimento encontrada
                </td>
              </tr>
            ) : (
                ...
                <tr key={order.id} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                  ...
                  {columnVisibility.tempoAtendimento && (
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 text-center">
                      {order.days_in_service} {order.days_in_service === 1 ? 'dia' : 'dias'}
                    </td>
                  )}
                </tr>
              ))
      ...
      <div className="mt-4 text-sm text-gray-600">
        Total de OPs em atendimento: {totalItems}
      </div>
    </div>
  );
}